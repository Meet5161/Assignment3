...
class Calculate():
    def calcDiscount(self, quantity_shirts, num_of_Trouser, calDiscount, calTotal):

        self.quantity_shirts = quantity_shirts
        self.num_of_Trouser = num_of_Trouser
        self.calDiscount = calDiscount
        self.calTotal = calTotal

bill = float (VOT + VOTS)

print ("\n Now select your category from below options ")
...
if a==1 or a==2:
    print("nice you have additional 10% discount")
    if quantity>=3:
        print("15 % on that")
        calDiscount = float(bill*0.25)
    else:
        calDiscount = float(bill*0.10)
else:
    print(" OOPS!! ")
    if quantity>=3:
        print("15 % on that")
        calDiscount = float(bill*0.15)
    else:
        calDiscount = 0

demo_bill = float (bill - calDiscount)

calTotal = float (demo_bill + demo_bill*0.13)

print (" \n Final amount is below after adding all the taxes  ")
